...
import { bankCashService, referenceService } from '../services/api'
...
const BankCash = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const response = await bankCashService.getMovements(params)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) {
        await bankCashService.updateMovement(editingItem.id, submitData)
      } else {
        await bankCashService.createMovement(submitData)
      }
      handleClose()
      clearDraft()
  ...
  const handleDelete = async (id: number) => {
    ...
    if (!confirmed) return
    try {
      await bankCashService.deleteMovement(id)
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await bankCashService.deleteMultiple(ids)
      ...
      >
        <form onSubmit={handleSubmit}>
            ...
            <div className="form-row">
              ...
              <FormField label="Счет списания" required error={validation.errors.payment_place_id}>
                <select
                    ...
                    <td>
                      {movement.movement_type === 'income' ? (
                        <span style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#27ae60' }}>
                          <HiOutlineArrowDownCircle style={{ fontSize: '18px' }} />
                          Поступление
                        </span>
                      ) : (
                        <span style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#e74c3c' }}>
                          <HiOutlineArrowUpCircle style={{ fontSize: '18px' }} />
                          Оплата
                        </span>
                      )}
                    </td>
